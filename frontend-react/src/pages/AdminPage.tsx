...
import type { Answer, AnswerSlot, HotspotConfig, MapConfig, MatchingPair, OrderItem, SelectionType } from "../api/quiz-api";
import "../admin.css";
...
export function AdminPage() {
  ...
  return <AdminEditor onLogout={() => setAuth("anonymous")} message={message} setMessage={setMessage} />;
}
...
function AdminEditor({ onLogout, message, setMessage }: { onLogout: () => void; message: string; setMessage: (value: string) => void }) {
  ...
  return <main className="admin-shell">
    ...
    {view === "chapters" && <section className="admin-panel"><div className="subject-tabs" aria-label="Przedmiot">{subjects.map((item) => <button aria-pressed={item.id === subject?.id} className="subject-tab" key={item.id} onClick={() => setSubject(item)} type="button">{item.title}</button>)}</div><div className="section-header"><h2>Rozdziały: {subject?.title ?? "Przedmiot"}</h2><button onClick={addChapter} type="button">+ Dodaj rozdział</button></div><ItemList empty="Brak rozdziałów. Dodaj pierwszy rozdział." items={chapters} onSelect={chooseChapter} /></section>}
    {view === "topics" && <section className="admin-panel"><div className="section-header"><h2>{chapter?.title ?? "Tematy"}</h2><button onClick={addTopic} type="button">+ Dodaj temat</button></div><ItemList empty="Ten rozdział nie ma jeszcze tematów. Dodaj pierwszy temat." items={topics} onSelect={chooseTopic} /></section>}
    {view === "questions" && <section className="admin-panel"><div className="section-header"><div><h2>{topic?.title ?? "Pytania"}</h2><p className="question-count">{formatQuestionCount(questions.length)}</p></div><button onClick={() => setEditorQuestion(null)} type="button">+ Dodaj pytanie</button></div>{questions.length === 0 ? <p className="empty-state">Ten temat nie ma jeszcze pytań. Dodaj pierwsze pytanie.</p> : <div className="question-list">{questions.map((item, index) => <div className="question-row" key={item.id}><span className="question-number">{index + 1}.</span><span className="question-text">{item.text}</span><span className="question-actions"><button className="secondary-button" onClick={() => setEditorQuestion(item)} type="button">Edytuj</button><button className="delete-button" onClick={() => deleteQuestion(item)} type="button">Usuń</button></span></div>)}</div>}</section>}
    {editorQuestion !== undefined && chapter && topic && <QuestionEditor question={editorQuestion} onCancel={() => setEditorQuestion(undefined)} onSave={async (payload) => { try { if (editorQuestion) await adminApi.updateQuestion(chapter.id, topic.id, editorQuestion.id, payload); else await adminApi.createQuestion(chapter.id, topic.id, payload); await loadQuestions(chapter, topic); setEditorQuestion(undefined); setMessage("Zapisano pytanie."); } catch (reason) { throw reason; } }} />}
  </main>;
}
...
function QuestionEditor({ question, onCancel, onSave }: { question: AdminQuestion | null; onCancel: () => void; onSave: (payload: AdminQuestionPayload) => Promise<void> }) {
  ...
  return <section className="editor-panel"><div className="editor-heading"><h2>{question ? "Edytuj pytanie" : "Dodaj pytanie"}</h2><p>{question ? "Zaktualizuj pytanie w wybranym temacie" : "Utwórz nowe pytanie w wybranym temacie"}</p></div><form id="question-form" onSubmit={submit}>
...
const questionTypes: Array<[SelectionType, string]> = [["single", "Jedna poprawna odpowiedź"], ["multiple", "Kilka poprawnych odpowiedzi"], ["open", "Odpowiedź pisemna"], ["order", "Układanie w kolejności"], ["matching", "Dopasowywanie par"], ["map", "Wskazanie na mapie"], ["hotspot", "Wskazanie na diagramie"], ["llm", "Odpowiedź pisemna oceniana przez AI"]];
