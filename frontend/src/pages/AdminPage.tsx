...
function AdminEditor({ onLogout, message, setMessage }: { onLogout: () => void; message: string; setMessage: (value: string) => void }) {
  ...
  return <main className="admin-shell">
    ...
    {!isQuestionEditorOpen && view === "topics" && <section className="admin-panel"><div className="section-header"><h2>{chapter?.title ?? "Tematy"}</h2><button onClick={addTopic} type="button">+ Dodaj temat</button></div><ItemList empty="Ten rozdział nie ma jeszcze tematów. Dodaj pierwszy temat." items={topics} onSelect={chooseTopic} /></section>}
