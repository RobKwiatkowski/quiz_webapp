...
export function TimedDivisionQuestion({ question, disabled, onComplete }: Props) {
  const [answer, setAnswer] = useState("");
  const [message, setMessage] = useState("");
  const timeLimitSeconds = question.timed_division_config?.time_limit_seconds ?? 10;
  ...
  const correctResult = question.quotient;

  useEffect(() => {
    if (disabled || dividend == null || divisor == null || correctResult == null) return;
    ...
    const timer = window.setInterval(() => {
      ...
      if (nextRemainingMs === 0 && !completedRef.current) {
        ...
        onCompleteRef.current({
          isCorrect: false,
          explanation: `Czas minął. ${question.explanation ?? `${dividend} : ${divisor} = ${correctResult}.`}`,
          earnedPoints: 0,
          maximumPoints: 1,
        });
      }
  ...
  }, [correctResult, disabled, dividend, divisor, question.explanation, question.id, timeLimitSeconds]);

  if (dividend == null || divisor == null || correctResult == null) {
  ...
  const submit = (event: FormEvent) => {
    ...
    onComplete({
      isCorrect,
      explanation: question.explanation ?? `${dividend} : ${divisor} = ${correctResult}.`,
